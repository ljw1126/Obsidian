...
## \[Frontend\]
**참고.**
- [벨로퍼트와 함께 하는 모던 리액트](https://react.vlpt.us/basic/10-useRef.html)
- [프론트맨 - 프론트엔ㅡ 개발자 면접 단골질문5](https://www.youtube.com/watch?v=rZrbQv1bMaI)
...
- 기본 제공
	...
	- 아래 컴포넌트 3가지는 불필요한 재랜더링을 방지하고 성능을 최적화하는데 사용
		...
		- `useEffect(.., [])`, `useMemo(.., [])`, `useCallback(.., [])`와 같이 두번째 dependency 인자에 상태값의 변경에 따라 재호출 여부 결정 (`[]`의 경우 최초 한번 실행)
- 커스텀 훅 
...
4. 리액트에서 `useEffect`를 사용할 때 **모범 사례**와 **주의할 점**을 설명해주세요 
	...
	2. dependency가 `[]`인 경우, 아닌 경우
		1. [f-lab, useEffect 이해와 활용](https://f-lab.kr/insight/understanding-useeffect-in-react?gad_source=1&gbraid=0AAAAACGgUFc_zmKhOr9ZWr9ZcuVGm78AD)
		2. dependency 설정을 통해 무한 루프 방지, unmount 때 실행되는 클린업 함수를 통해 메모리 누수를 방지하고 성능 최적화
5. 리액트에서 **코드 스플리팅**을 구현할 때 `React.lazy`와 `Suspense(정지)`를 어떻게 사용하나요? (기초 질문x)
	1. [리액트 공식문서](https://ko.legacy.reactjs.org/docs/code-splitting.html)
	2. React.lazy : 지연 처리해서 필요한 시점에 가져올 수 있다
	3. Suspence 컴포넌트 하위에 lazy 컴포넌트가 위치할 수 있다. (로딩바와 함께)
...
- ItemReader 구현체 중에 cursor와 paging기반이 있다 
	- Reader
		...
		- HibernateCursorItemReader / JpaPagingItemReader
		- StoredProcedureItemReader
	- Writer
		- JpaItemWriter 
		- JdbcBatchItemWriter

| 구분             | Cursor 기반         | Paging 기반             |
| -------------- | ----------------- | --------------------- |
| 쿼리 실행 방식       | 전체 쿼리 실행 후 커서 이동  | 페이지 단위로 반복 쿼리 실행      |
| Connection     | 커서 열려있는 동안 유지     | 각 페이지 쿼리마다 커넥션 사용     |
| 메모리 사용량(배치 기준) | 낮음(한번에 하나의 행 가져옴) | 높음 (한 페이지 단위)         |
| 처리방식           | 순차적 읽기            | 병렬 처리, 멀티 스레딩, 파티셔닝   |
| 성능             | 작은 데이터셋에 효율적      | 대량 데이터셋에 효율적          |
| 쿼리 복잡성         | 단순                | 복잡(`offest`, `limit`) |
| 데드락 위험         | 상대적으로 높음          | 낮음(페이지 단위로 처리해서)      |
| 트랜잭션 관리        | 장시간 커서 유지시 관리 힘듦  | 용이(페이지 단위로 처리해서)      |
| 재시작            | 어려움 (상태 저장 필요)    | 용이(페이지 단위로 처리해서)      |
- 방식/메모리/데드락/재시도/트랜잭션 
