- wsl2 docker 설치 
	- docker settings - general - wsl2 옵션 활성화


```shell
dotnet add package Testcontainers.Xunit
```
- 현재 xUnit v2를 사용 중이라서

```shell
dotnet add package Testcontainers.MsSql
```


### Container-per-test Strategy 
우선은 출력만 확인해본다..

```cs
using Testcontainers.MsSql;
using Xunit.Abstractions;

namespace ShipParticularsApi.Tests.Tests.Testcontainers
{
    public class DatabaseContainerPerTestcs(ITestOutputHelper output)
        : IAsyncLifetime
    {
        private readonly MsSqlContainer _dbContainer = new MsSqlBuilder()
            .WithImage("mcr.microsoft.com/mssql/server:2022-latest")
            .WithPassword("qwer1234!@#$")
            .WithCleanUp(true)
            .Build();

        private string connectionString = string.Empty;

        [Fact]
        public void Test()
        {
            output.WriteLine($"Connection String: {connectionString}");
        }

        public Task DisposeAsync()
        {
            return _dbContainer.StopAsync();
        }

        public async Task InitializeAsync()
        {
            await _dbContainer.StartAsync();

            connectionString = _dbContainer.GetConnectionString();
            output.WriteLine("InitializeAsync 호출");
        }
    }
}

```

`실행 결과`
```text
  표준 출력: 
InitializeAsync 호출
Connection String: Server=127.0.0.1,60764;Database=master;User Id=sa;Password=qwer1234!@#$;TrustServerCertificate=True
```


💩`Fluent Assertions 라이센스 이슈`
```text
 Warning:
 The component "Fluent Assertions" is governed by the rules defined in the Xceed License Agreement and
 the Xceed Fluent Assertions Community License. You may use Fluent Assertions free of charge for
 non-commercial use only. An active subscription is required to use Fluent Assertions for commercial use.
 Please contact Xceed Sales mailto:sales@xceed.com to acquire a subscription at a very low cost.
 A paid commercial license supports the development and continued increasing support of
 Fluent Assertions users under both commercial and community licenses. Help us
 keep Fluent Assertions at the forefront of unit testing.
 For more information, visit https://xceed.com/products/unit-testing/fluent-assertions/
```
- 상업용 사용시 법적 분쟁 발생 가능 
	- `Shouldy` 패키지 사용 권장✅ (`gemini`)
	- [shouldly/LICENSE.txt at master · shouldly/shouldly](https://github.com/shouldly/shouldly/blob/master/LICENSE.txt)

### DatabaseContainerPerTest


start와 stop이 4개 확인된다
```text
테스트 프로젝트 구축
요청한 테스트 실행에 대한 테스트 검색을 시작하는 중
========== 테스트 검색을 시작하는 중 ==========
[xUnit.net 00:00:00.00] xUnit.net VSTest Adapter v3.1.5+1b188a7b0a (64-bit .NET 8.0.21)
[xUnit.net 00:00:00.07]   Discovering: ShipParticularsApi.Tests
[xUnit.net 00:00:00.10]   Discovered:  ShipParticularsApi.Tests
========== 테스트 검색이 완료됨: 1.5초 동안 테스트 68개를 찾음 ==========
========== 테스트 실행을 시작하는 중 ==========
[xUnit.net 00:00:00.00] xUnit.net VSTest Adapter v3.1.5+1b188a7b0a (64-bit .NET 8.0.21)
[xUnit.net 00:00:00.08]   Starting:    ShipParticularsApi.Tests
[testcontainers.org 00:00:00.11] Connected to Docker:
  Host: npipe://./pipe/docker_engine
  Server Version: 28.5.1
  Kernel Version: 6.6.87.2-microsoft-standard-WSL2
  API Version: 1.51
  Operating System: Docker Desktop
  Total Memory: 15.41 GB
  Labels: 
    com.docker.desktop.address=npipe://\\.\pipe\docker_cli
[testcontainers.org 00:00:00.23] Docker container 430fc19985e1 created
[testcontainers.org 00:00:00.27] Start Docker container 430fc19985e1
[testcontainers.org 00:00:00.50] Wait for Docker container 430fc19985e1 to complete readiness checks
[testcontainers.org 00:00:00.51] Docker container 430fc19985e1 ready
[testcontainers.org 00:00:02.59] Docker container a206269925ea created
[testcontainers.org 00:00:02.60] Start Docker container a206269925ea
[testcontainers.org 00:00:02.76] Wait for Docker container a206269925ea to complete readiness checks
[testcontainers.org 00:00:02.77] Execute "/bin/sh -c find /opt/mssql-tools*/bin/sqlcmd -type f -print -quit" at Docker container a206269925ea
[testcontainers.org 00:00:02.87] Execute "/opt/mssql-tools18/bin/sqlcmd -C -Q SELECT 1;" at Docker container a206269925ea
[testcontainers.org 00:00:07.47] Docker container a206269925ea ready
     Warning:
     The component "Fluent Assertions" is governed by the rules defined in the Xceed License Agreement and
     the Xceed Fluent Assertions Community License. You may use Fluent Assertions free of charge for
     non-commercial use only. An active subscription is required to use Fluent Assertions for commercial use.
     Please contact Xceed Sales mailto:sales@xceed.com to acquire a subscription at a very low cost.
     A paid commercial license supports the development and continued increasing support of
     Fluent Assertions users under both commercial and community licenses. Help us
     keep Fluent Assertions at the forefront of unit testing.
     For more information, visit https://xceed.com/products/unit-testing/fluent-assertions/
[testcontainers.org 00:00:08.65] Stop Docker container a206269925ea
[testcontainers.org 00:00:10.01] Docker container e3fb68425d2f created
[testcontainers.org 00:00:10.02] Start Docker container e3fb68425d2f
[testcontainers.org 00:00:10.21] Wait for Docker container e3fb68425d2f to complete readiness checks
[testcontainers.org 00:00:10.23] Execute "/bin/sh -c find /opt/mssql-tools*/bin/sqlcmd -type f -print -quit" at Docker container e3fb68425d2f
[testcontainers.org 00:00:10.31] Execute "/opt/mssql-tools18/bin/sqlcmd -C -Q SELECT 1;" at Docker container e3fb68425d2f
[testcontainers.org 00:00:13.91] Docker container e3fb68425d2f ready
[testcontainers.org 00:00:14.13] Stop Docker container e3fb68425d2f
[testcontainers.org 00:00:15.48] Docker container 35bf95e6859a created
[testcontainers.org 00:00:15.49] Start Docker container 35bf95e6859a
[testcontainers.org 00:00:15.71] Wait for Docker container 35bf95e6859a to complete readiness checks
[testcontainers.org 00:00:15.72] Execute "/bin/sh -c find /opt/mssql-tools*/bin/sqlcmd -type f -print -quit" at Docker container 35bf95e6859a
[testcontainers.org 00:00:15.83] Execute "/opt/mssql-tools18/bin/sqlcmd -C -Q SELECT 1;" at Docker container 35bf95e6859a
[testcontainers.org 00:00:19.43] Docker container 35bf95e6859a ready
[testcontainers.org 00:00:19.81] Stop Docker container 35bf95e6859a
[testcontainers.org 00:00:21.23] Docker container f874f11bde11 created
[testcontainers.org 00:00:21.24] Start Docker container f874f11bde11
[testcontainers.org 00:00:21.42] Wait for Docker container f874f11bde11 to complete readiness checks
[testcontainers.org 00:00:21.43] Execute "/bin/sh -c find /opt/mssql-tools*/bin/sqlcmd -type f -print -quit" at Docker container f874f11bde11
[testcontainers.org 00:00:21.50] Execute "/opt/mssql-tools18/bin/sqlcmd -C -Q SELECT 1;" at Docker container f874f11bde11
[testcontainers.org 00:00:25.08] Docker container f874f11bde11 ready
[testcontainers.org 00:00:25.32] Stop Docker container f874f11bde11
[xUnit.net 00:00:26.86]   Finished:    ShipParticularsApi.Tests
========== 테스트 실행이 완료됨: 26.9초 동안 테스트 4개가 실행됨(4개 통과, 0개 실패, 0개 건너뜀) ==========

```



### 트러블 슈팅1
컨테이너 테스트 실행시 Repository 테스트를 그대로 복사해서 수행했는데 4개중 3개가 실패함
```text
[xUnit.net 00:00:10.05]       Microsoft.EntityFrameworkCore.DbUpdateException : An error occurred while saving the entity changes. See the inner exception for details.

[xUnit.net 00:00:10.05]       ---- Microsoft.Data.SqlClient.SqlException : Cannot insert explicit value for identity column in table 'SHIP_INFO' when IDENTITY_INSERT is set to OFF.
```


**🚨 오류 원인 분석: IDENTITY_INSERT(Gemini)**
로그에 명확하게 나와 있는 SqlException 메시지가 문제의 원인입니다.

Cannot insert explicit value for identity column in table 'SHIP_INFO' when IDENTITY_INSERT is set to OFF.

문제: 테이블 SHIP_INFO에 ID (기본 키) 컬럼이 SQL Server에서 자동 증가(Identity) 속성으로 설정되어 있습니다.

발생 시점: 테스트 코드에서 DbContext.Add()를 호출하여 엔터티를 삽입할 때, 엔터티에 ID 값을 직접 지정했기 때문에 SQL Server가 이를 거부한 것입니다.

인메모리 SQLite 환경에서는 자동 증가 설정이 느슨하게 처리되거나 아예 무시되었을 수 있지만, Testcontainers를 사용하는 실제 SQL Server 인스턴스는 이 제약을 엄격하게 적용합니다.\


✅ 해결 : Test Fixture에서 ID 값을 삭제

### 트러블 슈팅2

```text
Microsoft.EntityFrameworkCore.DbUpdateException : An error occurred while saving the entity changes. See the inner exception for details.

---- Microsoft.Data.SqlClient.SqlException : String or binary data would be truncated in table 'master.dbo.SHIP_INFO', column 'SHIP_KEY'. Truncated value: 'UNIQUE_SHI'.
```


- **문제:** 엔터티 `SHIP_INFO`의 **`SHIP_KEY`** 컬럼에 저장하려는 문자열 값이 **데이터베이스 테이블에 설정된 최대 길이**를 초과했기 때문에 SQL Server가 저장을 거부한 것입니다.
    
- **발생 이유:** 기존에 사용하시던 **인메모리 SQLite**는 문자열 길이 제한에 대해 매우 관대하거나 아예 무시합니다. 하지만 **Testcontainers의 SQL Server**는 프로덕션 환경처럼 **`NVARCHAR(50)`**과 같은 길이를 엄격하게 체크합니다.

✅ 해결 : ShipKey 문자열 길이 수정하여 테스트 4개 통과


🤔 SQLite로 테스트할때는 통과하던게 MSSQL 컨테이너 테스트에서는 실패하는가?

|**특징**|**SQLite (인메모리)**|**SQL Server (Testcontainers)**|
|---|---|---|
|**데이터 타입 검사**|**매우 느슨함** (유연함)|**매우 엄격함**|
|**문자열 길이 제한**|`VARCHAR`, `NVARCHAR` 길이 제한을 **무시**하거나 **적용하지 않음**.|`NVARCHAR(N)`에 명시된 길이를 **엄격하게 적용**. (👉 `Truncation` 오류 발생 원인)|
|**자동 증가(Identity)**|설정이 없어도 ID를 자동으로 처리하는 경우가 많음.|`IDENTITY_INSERT` 설정이 꺼져 있으면 **명시적인 ID 삽입을 엄격하게 거부**. (👉 `DbUpdateException` 오류 발생 원인)|
|**트랜잭션/잠금**|단일 파일 기반이라 복잡한 동시성 처리 로직이 단순함.|실제 다중 사용자 환경의 복잡한 잠금/트랜잭션 관리가 동작함.|

**🚨 발생한 두 가지 오류의 근본 원인 요약**

사용자님께서 겪으신 두 가지 오류는 모두 **"실제 DB의 엄격한 제약"** 때문에 발생한 것입니다.

1. **`IDENTITY_INSERT` 오류:** (이전 문제)
    - **원인:** SQLite는 관대했지만, SQL Server는 ID 컬럼에 **개발자가 값을 할당하는 것**을 허용하지 않았습니다.
    - **조치:** 엔터티를 삽입할 때 `Id = ...` 코드를 제거했습니다.
        
2. **`Data Truncation` (데이터 잘림) 오류:** (가장 최근 문제)
    - **원인:** SQLite는 관대했지만, SQL Server는 `SHIP_KEY` 컬럼의 **길이 제한**을 엄격하게 적용했습니다.
    - **조치:** `SHIP_KEY`에 할당하는 문자열의 길이를 DB 스키마에 맞게 줄여야 합니다.


### 고민 
`@DataJpaTest`의 경우 각 테스트 메서드마다 트랜잭션 롤백을 지원한다. 
- [DataJpaTest (Spring Boot 3.5.7 API)](https://docs.spring.io/spring-boot/api/java/org/springframework/boot/test/autoconfigure/orm/jpa/DataJpaTest.html)

그런데 현재 내가 하고 있는 Repository의 경우 컨테이너를 시작/종료하기 때문에 특별히 트랜잭션을 실행하거나 롤백하지 않고 있다.(명시적으로)

SaveChangeAsync() 통해 묵시적으로 트랜잭션을 보내는데 .. 전역 변수로 context와 transaction을 뽑아서 관리하는게 맞는지 테스트 메서드마다 개별적으로 트랜잭션 관리해주는게 맞는지 고민 🤔


`리팩터링 전`
```cs
public class DatabaseContainerPerTestcs(ITestOutputHelper output)
    : IAsyncLifetime
{
    private readonly MsSqlContainer _dbContainer = new MsSqlBuilder()
        .WithImage("mcr.microsoft.com/mssql/server:2022-latest")
        .WithPassword("qwer1234!@#$")
        .WithCleanUp(true)
        .Build();

    private DbContextOptions<ShipParticularsContext> _options;

    private ShipParticularsContext _context;
    private IDbContextTransaction _transaction;

    private ShipParticularsContext CreateContext() => new(_options);

    [Fact(DisplayName = "DB에서 조회한 엔티티는 DbContext가 상태 추적 한다.")]
    public async Task AsTracking()
    {
        // Arrange
        const string shipKey = "SHIP_KEY";

        _context.ShipInfos.Add(NoService(shipKey).Build());
        await _context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(_context);
        var actual = await repository.GetByShipKeyAsync(shipKey);

        actual.Should().NotBeNull();

        var entry = _context.Entry(actual!);
        entry.State.Should().Be(EntityState.Unchanged);
    }

    [Fact(DisplayName = "AsNoTracking()으로 조회한 엔티티는 상태추적하지 않는다.")]
    public async Task AsNoTracking()
    {
        // Arrange
        const string shipKey = "SHIP_KEY";

        _context.ShipInfos.Add(NoService(shipKey).Build());
        await _context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(_context);
        var actual = await repository.GetReadOnlyByShipKeyAsync(shipKey);

        actual.Should().NotBeNull();

        var entry = _context.Entry(actual!);
        entry.State.Should().Be(EntityState.Detached);
    }

    [Fact(DisplayName = "shipKey에 해당하는 선박 정보가 있으면 true를 반환한다")]
    public async Task ExistsByShipKeyAsync()
    {
        // Arrange
        const string shipKey = "SHIP_KEY";

        _context.ShipInfos.Add(NoService(shipKey).Build());
        await _context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(_context);
        bool actual = await repository.ExistsByShipKeyAsync(shipKey);

        actual.Should().BeTrue();
    }

    [Fact(DisplayName = "shipKey에 해당하는 선박 정보가 없으면 false를 반환한다")]
    public async Task None_ExistsByShipKeyAsync()
    {
        var repository = new ShipInfoRepository(_context);

        bool actual = await repository.ExistsByShipKeyAsync("SHIP_KEY");

        actual.Should().BeFalse();
    }

    public async Task DisposeAsync()
    {
        if (_transaction != null)
        {
            await _transaction.RollbackAsync();
            await _transaction.DisposeAsync();
        }
        await _context.DisposeAsync();
        await _dbContainer.StopAsync();
    }

    public async Task InitializeAsync()
    {
        await _dbContainer.StartAsync();

        _options = new DbContextOptionsBuilder<ShipParticularsContext>()
                       .UseSqlServer(_dbContainer.GetConnectionString())
                       .UseLazyLoadingProxies()
                       .Options;

        _context = new ShipParticularsContext(_options);
        _context.Database.Migrate();

        _transaction = await _context.Database.BeginTransactionAsync();
    }
}
```


```cs
public class DatabaseContainerPerTestcs(ITestOutputHelper output)
    : IAsyncLifetime
{
    private readonly MsSqlContainer _dbContainer = new MsSqlBuilder()
        .WithImage("mcr.microsoft.com/mssql/server:2022-latest")
        .WithPassword("qwer1234!@#$")
        .WithCleanUp(true)
        .Build();

    private DbContextOptions<ShipParticularsContext> _options;

    private ShipParticularsContext CreateContext() => new(_options);

    [Fact(DisplayName = "DB에서 조회한 엔티티는 DbContext가 상태 추적 한다.")]
    public async Task AsTracking()
    {
        // Arrange
        await using var context = CreateContext();
        await using var transaction = await context.Database.BeginTransactionAsync();

        const string shipKey = "SHIP_KEY";

        context.ShipInfos.Add(NoService(shipKey).Build());
        await context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(context);
        var actual = await repository.GetByShipKeyAsync(shipKey);

        actual.Should().NotBeNull();

        var entry = context.Entry(actual!);
        entry.State.Should().Be(EntityState.Unchanged);

        // 🌟 await using 블록 종료 시, transaction이 롤백되어 데이터 자동 정리.
    }

    [Fact(DisplayName = "AsNoTracking()으로 조회한 엔티티는 상태추적하지 않는다.")]
    public async Task AsNoTracking()
    {
        // Arrange
        await using var context = CreateContext();
        await using var transaction = await context.Database.BeginTransactionAsync();

        const string shipKey = "SHIP_KEY";

        context.ShipInfos.Add(NoService(shipKey).Build());
        await context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(context);
        var actual = await repository.GetReadOnlyByShipKeyAsync(shipKey);

        actual.Should().NotBeNull();

        var entry = context.Entry(actual!);
        entry.State.Should().Be(EntityState.Detached);
    }

    [Fact(DisplayName = "shipKey에 해당하는 선박 정보가 있으면 true를 반환한다")]
    public async Task ExistsByShipKeyAsync()
    {
        // Arrange
        await using var context = CreateContext();
        await using var transaction = await context.Database.BeginTransactionAsync();

        const string shipKey = "SHIP_KEY";

        context.ShipInfos.Add(NoService(shipKey).Build());
        await context.SaveChangesAsync();

        // Act & Assert
        var repository = new ShipInfoRepository(context);
        bool actual = await repository.ExistsByShipKeyAsync(shipKey);

        actual.Should().BeTrue();
    }

    [Fact(DisplayName = "shipKey에 해당하는 선박 정보가 없으면 false를 반환한다")]
    public async Task None_ExistsByShipKeyAsync()
    {
        await using var context = CreateContext();
        await using var transaction = await context.Database.BeginTransactionAsync();

        var repository = new ShipInfoRepository(context);

        bool actual = await repository.ExistsByShipKeyAsync("SHIP_KEY");

        actual.Should().BeFalse();
    }

    public async Task DisposeAsync()
    {
        await _dbContainer.StopAsync();
    }

    public async Task InitializeAsync()
    {
        await _dbContainer.StartAsync();

        _options = new DbContextOptionsBuilder<ShipParticularsContext>()
                       .UseSqlServer(_dbContainer.GetConnectionString())
                       .UseLazyLoadingProxies()
                       .Options;

        var context = new ShipParticularsContext(_options);
        context.Database.Migrate();
    }
```


### 비동기 using 선언 (Asynchronous Using Declaration)
- 이를 통해 `리소스 스코프 관리` 또는 `객체 수명 주기 관리 패턴`이라고 지칭


💡 이 패턴의 역할과 이점 (Repository 테스트에서의 의미)
이 패턴은 특히 데이터베이스 관련 테스트에서 두 가지 중요한 역할을 수행합니다.

1. 안전한 비동기 리소스 정리 (IAsyncDisposable 구현)
배경: DbContext나 IDbContextTransaction과 같은 DB 관련 리소스는 **비동기(Asynchronous)**로 정리(Dispose)되어야 할 로직(예: 비동기 트랜잭션 롤백, 비동기 연결 해제)을 포함하고 있습니다.

역할: await using 블록이 종료되면, C# 컴파일러는 자동으로 객체의 DisposeAsync() 메서드를 호출합니다.

코드 의미: "이 블록이 끝나면 arrangeContext 객체를 **비동기적으로 안전하게 정리(Clean up)**하라."

2. 격리된 스코프 보장 (Separate Context for Arrange)
배경: 통합 테스트에서 데이터를 준비(Arrange)하는 행위와 실제로 테스트 대상 Repository를 실행(Act)하는 행위는 서로 다른 DbContext 인스턴스에서 이루어져야 완벽한 격리가 보장됩니다.

역할: arrangeContext는 오직 데이터를 삽입하고 저장하는 역할만 수행하며, 블록이 끝나면 즉시 정리되어 Act 단계의 DbContext에 영향을 주지 않습니다.


```cs
// Arrange (데이터 삽입)
await using (var arrangeContext = CreateContext())
{
    // 1. 데이터를 삽입하고 커밋 (Act Context와 분리된 Context 사용)
    arrangeContext.ShipInfos.Add(/* ... */);
    await arrangeContext.SaveChangesAsync();
} // arrangeContext는 여기서 정리됨

// Act (테스트 실행)
await using (var actContext = CreateContext())
{
    var repository = new ShipInfoRepository(actContext);
    // 2. 새로운 actContext를 사용하여 Repository를 테스트
    // arrangeContext가 정리되었으므로 Act에 영향을 미치지 않음
}
```

결론적으로, 이 패턴은 통합 테스트에서 필수적인 "격리된 데이터 준비"와 "비동기 리소스의 안전한 정리"를 C# 문법을 통해 가장 우아하게 구현하는 방법입니다.


### 테스트 skip 방법 
[c# - Skipping a whole test class in xUnit.net - Stack Overflow](https://stackoverflow.com/questions/14840172/skipping-a-whole-test-class-in-xunit-net)

> 클래스 레벨에서 skip 하는 것은 지원하지 않는다
> 메타를 표기해두고 dotnet cli 로 실행시 제외하는 방법 있다는데 .. 번거로움


### Container-per-class Strategy 


🤔 테스트 실행시 컨테이너 시작 로그가 두번 찍힌다.
```text
[testcontainers.org 00:00:00.22] Docker container 89e57ca4675e created
[testcontainers.org 00:00:00.25] Start Docker container 89e57ca4675e
[testcontainers.org 00:00:00.43] Wait for Docker container 89e57ca4675e to complete readiness checks
[testcontainers.org 00:00:00.44] Docker container 89e57ca4675e ready
[testcontainers.org 00:00:02.52] Docker container 2810f21daa19 created
[testcontainers.org 00:00:02.53] Start Docker container 2810f21daa19
```

🤖`Gemini`
아, 흥미롭고 중요한 현상을 발견하셨습니다\!

분명히 테스트 클래스 하나에서 `MsSqlContainer` 인스턴스를 하나만 생성하고 `IAsyncLifetime`을 구현했는데, 로그에는 \*\*두 개의 Docker 컨테이너가 시작(Start Docker container)\*\*되는 것처럼 보입니다.

이는 일반적으로 **Testcontainers가 DB 컨테이너를 시작하기 위해 내부적으로 사용하는 보조 컨테이너** 때문일 가능성이 매우 높습니다.


**💡 로그에 두 개의 컨테이너가 보이는 이유 (Ryuk과 Init 컨테이너)**

Testcontainers는 개발자가 지정한 주력 컨테이너(여기서는 SQL Server) 외에도 테스트 환경을 관리하기 위해 보조 컨테이너를 사용합니다.

1.  **주력 컨테이너 (SQL Server):**
      * 로그에서 **오래 실행되고 실제로 DB 작업을 수행**하는 컨테이너입니다. (예: `89e57ca4675e`)
2.  **보조 컨테이너 (Ryuk 또는 Init):**
      * **Ryuk 컨테이너:** Testcontainers가 시작될 때, 테스트 실행 후 정리되지 않은 **모든 컨테이너와 볼륨을 안전하게 정리**하기 위해 Testcontainers 라이브러리가 자동으로 시작하는 작은 컨테이너입니다. 이는 Testcontainers를 사용하는 **세션당 한 번**만 시작되며, 테스트가 끝나도 바로 종료되지 않고 백그라운드에서 실행됩니다.
      * **Init 컨테이너:** 드물게, Testcontainers가 복잡한 초기화(예: 네트워크 설정, 라이선스 체크)를 위해 임시로 컨테이너를 시작하고 즉시 종료할 수도 있습니다.

**사용자님의 로그 분석:**

```
[testcontainers.org 00:00:00.25] Start Docker container 89e57ca4675e // 🌟 1. 첫 번째 컨테이너 시작
...
[testcontainers.org 00:00:02.53] Start Docker container 2810f21daa19 // 🌟 2. 두 번째 컨테이너 시작
```

이 두 번째 컨테이너(`2810f21daa19`)는 아마도 **Ryuk 컨테이너**일 가능성이 큽니다. 이 컨테이너는 DB 작업과는 무관하며, \*\*테스트 환경의 건전성(Cleanup)\*\*을 위해 존재합니다.

**✅ 결론: 걱정할 필요는 없습니다.**
  * 테스트 코드에서 **`new MsSqlContainer()`를 한 번만 호출**했다면, 여러분의 테스트 로직에 사용되는 DB 컨테이너는 하나입니다.
  * 나머지 컨테이너는 **Testcontainers 라이브러리 자체가 안정성을 위해 관리**하는 내부 도구이므로, **테스트 로직에 영향을 미치지 않습니다.**

**🚀 다음 단계: 성능 최적화로 전환**

이러한 내부 컨테이너의 오버헤드를 줄일 수는 없지만, 테스트가 느려지는 주범인 **SQL Server 컨테이너의 잦은 시작/종료**는 막아야 합니다.

이제 논의했던 대로 **컨테이너를 솔루션 전체에서 공유**하고 테스트 속도를 높이는 `ICollectionFixture` 패턴으로 전환하는 리팩토링을 시작하겠습니다.

**✅ 다음 답변에서 `ICollectionFixture`를 사용한 3가지 클래스 구조를 제시해 드리겠습니다.**


🤔 하나의 클래스 내에 컨테이너 하나가 공유되는지 확실히 알고 싶어서 임의의 테스트를 또 만듦 

```cs
public class DatabaseContainerPerTestClass2(DatabaseFixture fixture, ITestOutputHelper output)
    : IClassFixture<DatabaseFixture>, IDisposable
{
    public void Dispose()
    {
        output.WriteLine($"Container Id = {fixture.ContainerId}");
    }

    [Fact]
    public void SampleTest()
    {
        // Sample test implementation
        output.WriteLine("This is a sample test.");
    }
}
```

`출력 결과`
```text
// DatabaseContainerPerTestClass (4개 테스트 전부다)
Container Id = 2f80250e5cde7e727ce0c22ccc9a12157ce1d1c4f0a88684dfada00605ffa562

// DatabaseContainerPerTestClass2
Container Id = ad64d4ab8a6885846c1e09f053d6871e7e45ff4026692fac0aa2b67e556ba8d1
```
- 테스트 전체 실행시 컨테이너가 다른 것을 확인함


### IClassFixture


### Container-per-collection Strategy
- `[CollectionDefinition(..)]`으로 선언 
	- 같은 컨테이너를 공유할 테스트 클래스에서는 클래스 시그니처 위에 메타 추가 
	- `[Collection(..)]`

💩 `[CollectionDefinition(nameof(DatabaseCollection))]`, `[Collection(nameof(DatabaseCollection))]` 으로 설정시 테스트 클래스 3개의 컨테이너 아이디가 다른 이슈 (251104)


**🚨 오류 원인: `nameof()`와 `ICollectionFixture`의 해석 차이**
문제는 `[CollectionDefinition]`에 인자로 전달하는 **문자열**이 정확히 **일치**해야 xUnit이 리소스를 공유하는데, `nameof()`를 사용하면서 이 일치성이 깨진 것입니다.

1. `nameof(DatabaseCollection)` 사용 시 (오류 발생)
`nameof(DatabaseCollection)`는 컴파일 시점에 클래스 이름인 **"DatabaseCollection"** 이라는 문자열 리터럴로 치환됩니다.

```cs
// 🌟 CollectionDefinition의 인자는 문자열 리터럴 "DatabaseCollection"
[CollectionDefinition(nameof(DatabaseCollection))] 
public class DatabaseCollection : ICollectionFixture<DatabaseFixture>;
```
문제는 **다른 테스트 클래스**에서 컬렉션을 참조할 때 발생합니다.

```cs
// 🌟 여기에 사용된 "DatabaseCollection"은
//    nameof(DatabaseCollection)이 아닌, 'nameof(DatabaseCollection).GetType().Name'으로
//    해석되거나, xUnit의 메타데이터 로더가 인스턴스를 찾지 못하고
//    컬렉션 이름이 다른 값으로 해석될 가능성이 있습니다.
//    (특히 C# 9.0 이상의 Record, Init-only setter 등 새로운 구문과 섞일 때)
[Collection(nameof(DatabaseCollection))]
```

가장 큰 가능성은: xUnit이 nameof(T)로 정의된 컬렉션을 로드할 때, 컴파일러가 생성한 심볼 이름을 정확히 매칭하지 못하고, 컬렉션이 정의되지 않았다고 판단하여 각 테스트 클래스마다 새 컬렉션을 임시로 생성해 버렸을 수 있습니다.



✅ `nameof()`로 지정하지 않고 아래와 같이 **문자열** 고정하여 실행한 결과, 테스트 클래스 3개다 같은 컨테이너 사용하는 것을 확인함
```cs
// 🌟 CollectionDefinition의 인자가 공백이 포함된 문자열 리터럴 "Database Collection"
[CollectionDefinition("Database Collection")] 
public class DatabaseCollection : ICollectionFixture<DatabaseFixture>;

// 🌟 Collection의 인자도 정확히 동일한 문자열 리터럴 "Database Collection"
[Collection("Database Collection")]
public class DatabaseContainerPerCollection(...)
```
이 경우에는 인자가 단순한 **문자열 리터럴**로 정확히 일치하며, xUnit은 문자열 비교만으로 **"Database Collection"이라는 이름의 Fixture 인스턴스가 이미 존재한다"**는 것을 확실하게 알 수 있습니다.


```cs
[CollectionDefinition("Database Collection")]
public class DatabaseCollection : ICollectionFixture<DatabaseFixture>;

[Collection("Database Collection")]
public class DatabaseContainerPerCollection(DatabaseFixture fixture, ITestOutputHelper output)
    : IDisposable
{
    public void Dispose()
    {
        output.WriteLine($"Collection Container Id = {fixture.ContainerId}");
    }
 
	// 테스트 코드  
}
```


```cs
[Collection("Database Collection")]
public class DatabaseContainerPerCollection2(DatabaseFixture fixture, ITestOutputHelper output)
    : IDisposable
{
    public void Dispose()
    {
        output.WriteLine($"Collection Container Id = {fixture.ContainerId}");
    }
	 
	// 테스트 코드   
}

```


`콘솔 출력`
```text
// DatabaseContainerPerCollection.cs
Collection Container Id = bcca2c060690020ef020a95be21e3402ebd2967b271b20e0be2b3468146734aa

// DatabaseContainerPerCollection2.cs
Collection Container Id = bcca2c060690020ef020a95be21e3402ebd2967b271b20e0be2b3468146734aa

//DatabaseContainerPerCollection3.cs
Collection Container Id = bcca2c060690020ef020a95be21e3402ebd2967b271b20e0be2b3468146734aa
```

> nameof(..)가 편리하긴 하지만 컴파일 시점에 클래스마다 다르게 해석할 수 있다. 내 생각에 제트브레인 예시의 경우 static class 안에 두 개의 클래스를 선언해서 작성한 예제이다 보니 컴파일 시점에 통일되서 문제 없이 실행 된 것으로 생각 된다.

**📌참고. 제트브레인 문서** 
[How to use Testcontainers with .NET Unit Tests | The .NET Tools Blog](https://blog.jetbrains.com/dotnet/2023/10/24/how-to-use-testcontainers-with-dotnet-unit-tests/#container-per-collection-strategy)

> ShipInfoBaseRepositoryTest를 작성했는데.. 기존에 Testcontainers 테스트 한거랑 똑같음 결국 ㅋㅋ 삽질만..... IAsyncLifetime은 클래스 단위로 생성/소멸 하다보니 DbContext와 Transaction을 여기서 관리하게 되면 모든 테스트 메서드가 같은 트랜잭션을 공유하게 됨 => 그래서 테스트메서드에서 context, transaction 생성하고 관리하는 방식으로 다시 변경

---

### 서비스 레이어 통합 테스트시 외부 트랜잭션 롤백 실행시 에러 발생



`방법1.` .. 서비스 실행하기 전에 외부에서 트랜잭션으로 감싸는 방식
스프링 생각하여 트랜잭션 전파(참여)하지 않을까 기대했는데 TransactionDecorator에서 트랜잭션 시작시 에러 발생
> System.InvalidOperationException : The connection is already in a transaction and cannot participate in another transaction.

`방법2.` 외부 트랜잭션 생성해서 공유 (안됨 x)

`방법3.` SQLite 사용한 통합테스트에서 사용한 DbInit 방식 (실패)

> Microsoft.Data.SqlClient.SqlException : Option 'SINGLE_USER' cannot be set in database 'master'.

`방법4.` respawn 라이브러리 사용하여 clean-up 
- [jbogard/Respawn: Intelligent database cleaner for integration tests](https://github.com/jbogard/Respawn?tab=readme-ov-file)
- [Easy Test Database Reset in .NET with Respawn | by Roko Kovač | Medium](https://medium.com/@kova98/easy-test-database-reset-in-net-with-respawn-d5a59f995e9d)
- [Faster .NET Database Integration Tests with Respawn and xUnit | Khalid Abuhakmeh](https://khalidabuhakmeh.com/faster-dotnet-database-integration-tests-with-respawn-and-xunit)
- [respawn-sample/Spawn/DatabaseFixture.cs at main · khalidabuhakmeh/respawn-sample](https://github.com/khalidabuhakmeh/respawn-sample/blob/main/Spawn/DatabaseFixture.cs)
- [testcontainers-dotnet/examples/Respawn/tests/Respawn.Tests/Usings.cs at develop · testcontainers/testcontainers-dotnet](https://github.com/testcontainers/testcontainers-dotnet/blob/develop/examples/Respawn/tests/Respawn.Tests/Usings.cs)

```shell
// ShipParticulars.Tests 디렉터리에서 실행
dotnet add package Respawn
```


### NTest, xUnit, JUnit5 테스트 이벤트 훅 관리
[Writing Tests in .NET with Respawn & TestContainers | Medium](https://medium.com/@hiddenhenry/simple-integration-testing-in-net-with-respawn-testcontainers-39f5de21740c)

DB clean-up 관련해서 찾아보다가 위의 블로그에 속성 기반으로 테스트 라이프 사이클을 관리하는 것을 발견

|**특징**|**xUnit (C#/.NET)**|**NUnit (C#/.NET)**|**JUnit 5 (Java)**|
|---|---|---|---|
|**Fixture/Setup**|**인터페이스 구현** (`IClassFixture`, `ICollectionFixture` 등)|**속성(Attribute)** 기반 (`[SetUp]`, `[OneTimeSetUp]` 등)|**애노테이션(Annotation)** 기반 (`@BeforeEach`, `@BeforeAll` 등)|
|**철학**|테스트 격리, "Convention over Configuration"|직관적인 속성 기반 설정|유연하고 확장성 높은 애노테이션 기반|
|**이벤트 훅**|인터페이스를 통한 명시적 연결|속성 및 상속 등을 통한 연결|Extension Model을 통한 유연한 확장|

xUnit은 IAsyncLifetime, IAsynsDispose 인터페이스의 이벤트 훅을 구현하여 테스트 라이프 사이클을 관리한다. 

반면 NUnit의 경우 JUnit5와 비슷하게, `[attribute]` 기반으로 선언해 관리하다보니 xUnit 보다 테스트 라이프 사이클 관리가 깔끔해 보인다.

---
### Reference
- [How to use Testcontainers with .NET Unit Tests | The .NET Tools Blog](https://blog.jetbrains.com/dotnet/2023/10/24/how-to-use-testcontainers-with-dotnet-unit-tests/)
- [Testing with xUnit.net - Testcontainers for .NET](https://dotnet.testcontainers.org/test_frameworks/xunit_net/)
- [How to Use TestContainers in .Net](https://www.freecodecamp.org/news/how-to-use-testcontainers-in-net/)
- [khalidabuhakmeh/TestingWithContainers: Using TestContainers.NET with Xunit for integration testing](https://github.com/khalidabuhakmeh/TestingWithContainers)
- [Sharing Context between Tests | xUnit.net](https://xunit.net/docs/shared-context)
	- xUnit 버전에 따라 생성자 초기화시 비동기 메서드 호출이 불가능하다는 내용을 언급 
		- 특히 v2는 안됨

