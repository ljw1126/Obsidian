...
🤔 TimerTrigger 실행시 콘솔에 아래와 같이 출력된다
```text
Host lock lease acquired by instance ID '..'
```

🔑 `Host Lock Lease Acquired`의 의미 (gemini)
이 메시지의 의미를 이해하려면 **'호스트 잠금(Host Lock)'** 개념을 알아야 합니다.

1. `호스트 잠금 (Host Lock Lease)`
Azure Functions는 여러 인스턴스(컴퓨터)에서 동시에 실행될 수 있습니다.
- **문제:** 타이머 트리거 함수가 여러 인스턴스에서 동시에 실행된다면, 하나의 스케줄 작업이 N번 중복 실행될 수 있습니다.
- **해결:** Functions 호스트는 **Azure Storage**를 사용하여 **분산 잠금(Distributed Lock)** 시스템을 구현합니다. 이 잠금을 **'리스(Lease)'**라고 부릅니다.

2. `Lease Acquired` 메시지의 역할

`Host lock lease acquired` 메시지는 다음과 같은 의미입니다.

| **메시지**              | **의미**                                                                       |
| -------------------- | ---------------------------------------------------------------------------- |
| **`Lease Acquired`** | "나(`instance ID`)는 지금부터 이 타이머 스케줄(또는 특정 싱글턴 작업)을 실행할 수 있는 **권한(잠금)**을 획득했다." |

이 잠금을 획득한 **단 하나의 인스턴스**만이 타이머 작업을 실행할 수 있습니다. 다른 모든 인스턴스들은 이 메시지가 뜨지 않으며, 잠금이 해제될 때까지 대기합니다.

3. `디버깅 환경에서의 동작`

로컬에서 실행할 때도 이 메시지는 동일하게 나타나며, 타이머가 올바르게 작동할 준비가 되었음을 의미합니다.
...
`251110`
- TestManagerDb 우선 조회
- spmsstorage 는 azurite 사용해서 queue에 저장 확인
- 인터페이스 추출해서 사용하도록 수정
	- queue client 는 connection string만 주입받아도 간단하게 재활용 가능한데
	- 전부다 static 처리가 되어 있다보니깐 사용하기 너무 어려운거 같다.
	- 불필요하게 사용하지 않는 곳에서도 static으로 올라가는게 아닌가 싶은데 .. 
- 그리고 타이머 트리거 자체 적으로 lock을 지원하고 있어서 플래그 비트가 의미 없어 보인다. 날짜 정보의 경우 싱글톤 형태로 갱신해서 사용하여 외부에서 테스트하기 쉽지 않을가 싶다.

> sentry를 우선 deprecated 된 것을 대체하긴 했는데 테스트하진 못했다.
> TestManagerDb를 직접 연결하고 있는데, docker로 mssql 올려야 하지 않나 싶다.
> 이때 필요한 테이블이 3개 정도 되는 듯 하다.



---
### 리팩터링 대상 
1. `TimerTrigger`에서 닷넷 8부터 함수는 락을 사용한다고 함. 그래서 플래그 변수가 필요 없는듯함 . 그리고 날짜 멤버 변수의 경우에도 테스트 하기 힘들게 만드는데, 싱글톤으로 해서 상태 변경하는 형태로 공유하는게 어떠한가 제시함 (gemini)

2. `IReadOnlyList<LOGGER_DATA_SETTING>`를 QueueTrigger에서는 사용하지 않는다. 특정 서비스 내부에서 사용하는데 .. 의존성 주입을 해당 서비스로 이관하거나 하면 좋을거 같다.


3. `Task<bool> Parsing**` 메서드 하나만 서비스에 public 하게 사용 중인데 반환값을 사용하지도 않는다..

✅ 서비스 인터페이스의 시그니처를 제거한다. 
✅ 10분 Queue 트리거에 있던 조회 로직 일부를 10분 분석 서비스로 이관
✅ DI 방식으로 전환하면서 파라미터 인자도 감소

4. 읽기 전용 조회인 경우 `AsNoTracking()` 상태가 되야 하는거 아닐까?


---
### Cosmos 설치 
- 공식 문서에서 지원하는 azure cosmos emulator 설치 
- `../도커` 디렉터리 참고
