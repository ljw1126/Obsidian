...
- [x] `repos/` 하위에 두 서비스 클론/동기화 ✅ 2026-02-01
- [x] Dockerfile 준비: `dockerfiles/Dockerfile-admin`, `dockerfiles/Dockerfile-reservation`. ✅ 2026-02-01
- [x] 앱 compose(`atdd-tests/docker/docker-compose.yml`)에 서비스를 추가한다. ✅ 2026-02-01
- [x] DB는 아래 3번에서 진행할 예정이니 지금은 무시한다. ✅ 2026-02-01
- [x] 200 응답 스모크 테스트를 통과시키다. ✅ 2026-02-01

**2) 애플리케이션 기동 및 e2e 테스트 통과**
- [x] 목적: `kiosk → admin → DB` 동작 여부를 확인한다. 각 애플리케이션을 기동해 전용 DB에 정상 연결되는지 확인하고 스모크/E2E 테스트를 통과시킨다. ✅ 2026-02-01
- [x] 시나리오 예시 ✅ 2026-02-01
...
@Value("${kiosk.admin.base-url}")  // 컨테이너 내부 통신을 위해 환경 변수 설정 필요
private String adminBaseUrl;  
...
```
- 도커 네트워크와 컨네이터명으로 통신할 수 있도록 환경 변수 설정이 필요하다 
- 인증 토큰은 kiosk 서버내에서 캐시로 관리하고 있는 상태이다.
- 고로 docker-compose 설정 후 e2e 테스트 시나리오를 잘 작성하면 된다


 **3) 공용 인프라 DB로 전환**

- [x] 공용 인프라 DB는 `@docker-compose-infra.yml`에 정의된 구성을 사용한다(`atdd-tests/docker/docker-compose-infra.yml`). ✅ 2026-02-01
- [x] 인프라 compose(docker-compose-infra.yml)를 먼저 기동해 공용 네트워크(`atdd-net`)와 DB(`atdd-db`)를 준비한다. ✅ 2026-02-01
- [x] 앱 compose에서 각 서비스를 공용 네트워크 `atdd-net`에 합류하도록 수정한다. ✅ 2026-02-01
- [x] 각 앱의 DB 접속 정보는 공용 DB `atdd-db`를 가리키도록 변경한다(환경변수 오버라이드 권장: `SPRING_DATASOURCE_URL`, `SPRING_DATASOURCE_USERNAME`, `SPRING_DATASOURCE_PASSWORD`). ✅ 2026-02-01
	- DB 초기화는 기존 `infra/db/init.sql`을 활용한다.
- 힌트: 데이터 초기화 시점은 infra를 띄우고 app을 실행하기 전에 수행해야 한다. 또 디비가 뜨기 전에 초기화 쿼리를 실행할 경우 동작하지 않으니 이 부분도 참고해야 한다.

```text
- admin, reservation 프로젝트 모두 `resources/data.sql`을 가지고 있다. 
- docker-compose-infra.yml정의된 mysql 컨터이너를 사용한다
- 다 같은 atdd-net에 소속된다
- db가 먼저 실행된 이후에 docker-compose-yml이 실행되어야 한다
- 그리고 db 실행 후에 admin, reservation에 data.sql이 실행이 되어야 하는데 .. 볼륨, 마운트를 유지해야 되나 싶기도 하다..
- admin, reservation에 mysql 의존성이 없다. 환경 변수 설정도 없고
- 
  
```
- docker-compose-infra.yml -> docker-compose.yml 실행 해야 한다
- 그리고 admin, reservation에 mysql 드라이버가 없다..
...
spring:  
  ...
  # 초기화 SQL(init.sql)은 이미 DB 컨테이너에서 실행되므로,  
  # 앱 차원에서의 SQL 초기화는 끌 수 있습니다.  
  sql:  
    init:  
      mode: never
...
환경 변수 추가
> tests 도커 컴포즈에는 SPRING_PROFILES_ACTIVE=prod 환경 변수를 추가한다

다시 테스트 프로젝트에서 git 이력을 최신화한다
```shell
✦ build.gradle.kts 파일의 내용을 성공적으로 업데이트했습니다.

  이제부터는 터미널에서 다음 명령어를 사용하여 전체 인프라와 애플리케이션
  서비스를 올바른 순서로 한 번에 관리할 수 있습니다.

   * 모든 서비스 시작:
   1     ./gradlew composeUp
      (실행 순서: InfraComposeUp -> ServiceComposeUp)

   * 모든 서비스 종료:
   1     ./gradlew composeDown
      (실행 순서: ServiceComposeDown -> InfraComposeDown)

  또한, 가독성을 위해 관련 태스크들을 docker 그룹으로 묶고 설명을 추가했습니다.
...
이제 이 구조를 개선해보도록 한다
- 1) build.gradle.kts 에서 tasks를 따로 관리하도록 한다

```shell
// 안된 💩
이제 프로젝트의 빌드 설정은 다음과 같이 세 개의 파일로 나뉘어 관리됩니다.

   1. `build.gradle.kts`: 프로젝트의 기본 정보(플러그인, 그룹, 버전 등)와 다른
      설정 파일을 불러오는 역할만 수행합니다.
   2. `gradle/dependencies.gradle.kts`: 모든 의존성 선언을 담당합니다.
   3. `gradle/tasks.gradle.kts`: 모든 커스텀 태스크 정의를 담당합니다.

  이렇게 구조를 변경함으로써 각 파일의 역할이 명확해져서 앞으로 빌드 설정을
  관리하기가 훨씬 수월해질 것입니다.

  작업이 모두 완료되었습니다. 다른 도움이 필요하시면 언제든지 말씀해주세요.
```


```shell
gradle/dependencies.gradle.kts:8:5: Unresolved reference: testImplementation
```
- 결국 의존성은 기존 build.gradle.kts로 이동 
- tasks의 경우 위에 `tasks.test { useJUnitPlatform() }` 있어서 오류 나는것 같아 이동하니 정상 동작함

- **2) repos에 clone > docker compose up (인프라, 서비스) > 테스트 실행 > 자원 정리**
	- `tasks.gradle.kts` 에다가 정의를 해버리네
- 3) 자동화 인수테스트 실행시 테스트 실패
	- 컨테이너 실행 후 서비스 시작까지 기다지 않고 요청해서 실패하는 걸로 보인다

```shell
 모든 수정이 완료되었습니다.

  이제 acceptanceTest 태스크의 실행 흐름은 다음과 같이 변경되었습니다.

   1. gitPullAll 태스크가 저장소들을 최신 상태로 업데이트합니다.
   2. composeUp 태스크가 Docker 컨테이너들을 시작합니다.
   3. `waitForServices` 태스크가 모든 서비스의 Health Check API를 주기적으로
      호출하며 서비스가 완전히 실행될 때까지 기다립니다.
   4. 모든 서비스가 준비되면 test 태스크가 실제 테스트를 시작합니다.
   5. 테스트가 끝나면 composeDown 태스크가 모든 환경을 정리합니다.

  이제 컨테이너 내부의 서비스가 완전히 구동된 후에 테스트가 시작되므로, 이전과
  같이 서비스가 준비되지 않아 테스트가 실패하는 문제를 해결할 수 있습니다.

  ./gradlew acceptanceTest 명령으로 전체 자동화 테스트를 실행해 보세요.
```
...
networks:
  atdd-net:
    external: true  // 외부에서 생성한 네트워크를 사용, 라이프 사이클을 docker-compose가 가지지 않음
```
...
// https://github.com/yhh1056/atdd-camping-tests/blob/step2/src/test/java/com/camping/tests/config/TestEnvironment.java
- 환경변수 설정이나 api, context가 좀 더 세분화되어 있다
---
