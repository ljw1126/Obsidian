...
**3. 성수기 할증 계산, 동시성 예약 분리**

```
✦ 네, 두 가지 모두 매우 중요한 포인트를 짚어주셨습니다. 테스트의 의도를 명확히 하고 각 테스트가
  올바른 수준(level)에서 책임을 다하도록 만드는 핵심적인 리팩토링입니다. 제안해주신 내용에 전적으로
  동의하며, 다음과 같이 구체적인 실행 계획을 안내해 드리겠습니다.

  1. '성수기 주말 할증' 테스트 리팩토링

  말씀하신 대로, 테스트가 특정 날짜에 종속되어 실패하는 것을 막고, given 절의 가독성을 높이는 것이
  중요합니다.

   - 문제점:
       1. 성수기 주말(e.g., 7월의 첫 토요일)을 계산하는 로직이 given 절에 그대로 노출되어 테스트의
          의도를 파악하기 어렵습니다.
       2. LocalDate.of(TODAY.getYear(), ...) 방식은 연도가 바뀌면 테스트가 실패할 수 있는 잠재적
          위험을 완전히 제거하지 못합니다. (예: 실행 시점이 이미 8월을 지난 경우)

   - 해결 방안:
       1. ReservationFixture에 성수기를 나타내는 PEAK_SEASON_START_MONTH, PEAK_SEASON_END_MONTH
          상수를 추가하여 "7월, 8월이 성수기"라는 비즈니스 규칙을 명시합니다.
       2. ReservationCreationAcceptanceTest 내에, "다음 성수기 기간의 첫 주말"을 동적으로 찾아주는
          private 헬퍼 메서드(e.g., findNextPeakSeasonWeekend())를 만듭니다. 이 메서드는 현재
          날짜를 기준으로 항상 미래의 성수기 주말을 찾아주므로 시간이 지나도 테스트가 깨지지
          않습니다.
       3. given 절에서는 이 헬퍼 메서드를 호출하여 간결하게 "성수기 주말이 주어졌을 때"라는
          시나리오를 표현합니다.

  2. '동시성 예약' 테스트 분리

  이 또한 아주 정확한 판단입니다. 동시성 제어는 사용자 시나리오라기보다는 기술적인 구현의 정확성을
  검증하는 것에 가깝습니다. 따라서 서비스 계층에 대한 통합 테스트로 분리하는 것이 가장
  이상적입니다.

   - 문제점: 인수 테스트는 사용자의 주요 흐름(Happy Path)과 예외 시나리오에 집중해야 하는데, 동시성
     테스트와 같은 기술 검증 코드가 섞여 있으면 테스트 스위트의 목적이 흐려집니다.
   - 해결 방안:
       1. ReservationServiceConcurrencyTest.java라는 새로운 서비스 통합 테스트 클래스를 생성합니다.
       2. ReservationCreationAcceptanceTest에 있던 동시성 테스트 로직을 이 클래스로 이전합니다.
       3. 테스트 로직을 API 호출(예약을_요청한다) 방식에서 서비스 레이어 직접
          호출(reservationService.reserve(...)) 방식으로 변경합니다. 이를 통해 웹 레이어를 거치지
          않고 서비스의 동시성 제어 로직만 정밀하게 테스트할 수 있습니다.
       4. 기존 ReservationCreationAcceptanceTest에서는 해당 테스트를 삭제하여 클래스를 순수한
          시나리오 테스트로 유지합니다.

  실행 계획

  이 계획에 따라 순차적으로 리팩토링을 진행하겠습니다.

   1. ReservationFixture에 성수기 월(month) 상수 추가.
   2. ReservationCreationAcceptanceTest에서 '성수기 주말 할증' 테스트 리팩토링.
   3. ReservationServiceConcurrencyTest 클래스 생성 및 동시성 테스트 이전.
   4. ReservationCreationAcceptanceTest에서 동시성 테스트 삭제
```


```text
 네, 완벽하게 이해했습니다. 시나리오 기반 테스트의 가독성을 극한으로 끌어올리는 매우 정교하고
  훌륭한 제안입니다.

  given 절에서 날짜를 계산하는 로직마저도 추상화하고, "성수기 주말에 예약을 요청한다" 는 비즈니스
  시나리오 문장 자체가 테스트 코드의 when 절이 되도록 만드는 것은 BDD(행위 주도 개발)의 이상적인
  모습과 정확히 일치합니다.

  이 제안을 반영하여 리팩토링을 진행하겠습니다.

  리팩토링 계획

   1. `ReservationStep.java`에 시나리오 기반의 고수준(high-level) 액션 메서드 추가:
       * 성수기_주말에_예약을_요청한다(String name, String phoneNumber) 라는 public static 메서드를
         새로 만듭니다.
       * 이 메서드 내부에 "다음 성수기 주말 날짜를 찾는 로직"과 "ReservationRequest 객체를 생성하는
         로직"을 모두 캡슐화합니다.
       * 마지막으로, 기존의 예약을_요청한다(request)를 호출하여 실제 API 요청을 보내고 그 결과를
         반환합니다.

   2. `ReservationCreationAcceptanceTest.java`를 단순화:
       * 성수기_주말_할증_요금이_자동_계산된다 테스트에서 기존의 복잡한 given-when 로직을 모두
         제거합니다.
       * 대신, 새로 만든 성수기_주말에_예약을_요청한다 메서드를 호출하는 단 한 줄의 when으로
         대체합니다.
       * 테스트 클래스 내부에 있던 findNextPeakSeasonWeekend 헬퍼 메서드는 더 이상 필요 없으므로
         삭제합니다.

  이 계획에 따라, 먼저 ReservationStep.java에 새로운 시나리오 메서드를 추가하는 것부터
  시작하겠습니다.
...
public interface CampsiteRepository extends JpaRepository<Campsite, Long> {
    ...
    Optional<Campsite> findBySiteNumberWithPessimisticLock(String siteNumber);
}
```
...
이제 모든 코드 수정이 완료되었습니다.
   ...
   - CampsiteRepository에는 비관적 락을 사용하는 findBySiteNumberWithPessimisticLock 메서드가
     추가되었습니다.
   - ReservationService의 createReservation 메서드는 이 새로운 메서드를 호출하여 예약 생성 과정에서
     Campsite row에 잠금을 겁니다.
   - ReservationServiceConcurrencyTest는 이 서비스 메서드를 호출하여 동시성 문제를 테스트합니다.

  이로써 비관적 락 적용이 완료되었습니다. 이제 ReservationServiceConcurrencyTest를 실행하면, 비관적
  락이 레이스 컨디션을 방지하여 한 건의 예약만 성공하고 테스트가 통과할 것입니다.

```

> spring data jpa가 처음에는 메서드 네이밍으로 쿼리를 만들려고 하는데, Lock이라는걸 몰라서 스프링 컨텍스트가 안뜬거였다 . @Query 붙여서 선언해주니 정상 동작한다

```
#### **비관적 락(Pessimistic Lock)의 한계점 인지**

현재 `PESSIMISTIC_WRITE`를 사용하셨는데, 이는 DB에 `SELECT FOR UPDATE` 쿼리를 날려 해당 행을 잠급니다. 이 방식은 데이터 정합성을 가장 확실하게 보장하지만, **대기 스레드가 많아지면 DB 커넥션 풀이 마를 수 있습니다.** 만약 나중에 트래픽이 엄청나게 늘어난다면 **낙관적 락(Optimistic Lock)**이나 **Redis 분산 락**으로의 전환을 고려해야 할 시점이 올 것입니다.

#### **데이터 클리너 위치**

지금 `setUp`에서 `databaseCleaner.execute()`를 호출하고 계신데, 이는 아주 안전한 방식입니다. 다만 `SITE_A1` 사이트를 등록하는 로직도 `base` 클래스로 옮기거나 `Fixture`화 하면 여러 테스트 클래스에서 사이트 등록 코드를 중복해서 작성하지 않아도 됩니다.
```


```
와, 날카로운 지점이네요! 그 현상은 JPA와 하이버네이트의 메서드 이름 분석(Query Derivation) 우선순위 때문에 발생하는 아주 전형적인 케이스입니다.

1. 왜 @Query가 없으면 컨텍스트가 안 떴을까요?
@QueryHints나 @Lock은 쿼리 실행 시의 옵션일 뿐, 그 자체로 쿼리를 만들어주지는 않습니다.

@Query가 없을 때: Spring Data JPA는 findBySiteNumberWithPessimisticLock이라는 메서드 이름을 보고 쿼리를 추출하려 합니다. 이때 엔티티에 siteNumberWithPessimisticLock이라는 필드가 없으므로, 구동 시점에 **"그런 필드 없는데?"**라며 에러를 던지고 멈춰버린 것입니다.

@Query를 추가했을 때: 메서드 이름 분석을 무시하고 작성하신 JPQL을 바로 사용하기 때문에 정상적으로 동작하게 된 것이죠.

2. QueryHints: 비관적 락의 완성
사용하신 javax.persistence.lock.timeout 힌트는 동시성 제어에서 매우 중요합니다.

역할: 다른 트랜잭션이 이미 락을 쥐고 있을 때, 무한정 기다리지 않고 **3초(3000ms)**만 기다린 후 예외를 발생시키라는 설정입니다.

주의: 이 힌트는 DB 방언(Dialect)에 따라 동작 여부가 다릅니다. (Oracle은 잘 되지만, PostgreSQL이나 MySQL의 특정 버전에서는 무시될 수도 있습니다.)

3. 실무형 리팩터링 제안
메서드 이름을 JPA 규칙에 맞게 줄이거나, 아예 의도를 명확히 하는 이름을 쓰고 @Query를 유지하는 것이 좋습니다.
...
💡 6년 차의 조언: 동시성 테스트의 완성도 높이기
이제 이 락이 실제 동시성 테스트에서 잘 작동하는지 확인하시려면, 테스트 코드에서 실패하는 케이스의 에러 메시지를 확인해 보세요.

락 타임아웃이 발생하면 PessimisticLockException이나 LockTimeoutException이 발생할 텐데, 이 예외가 발생했을 때 사용자에게 **"현재 예약 시도가 많으니 잠시 후 다시 시도해주세요"**라는 메시지를 보여주는 로직까지 갖춰진다면 완벽한 서비스가 됩니다.

혹시 동시성 테스트 결과가 successCount 1로 잘 나오나요? 만약 여전히 2명이 성공한다면, 락이 거리는 대상(Campsite)이 아니라 다른 곳(Reservation)을 잠그고 있는 건 아닌지 체크해 봐야 합니다. 진행 상황은 어떠신가요?

Would you like me to ...? (동시성 실패 시 발생하는 예외를 세련되게 처리하는 전역 예외 처리기(GlobalExceptionHandler) 예시를 보여드릴까요?)
```


----

> 테스트 데이터 빌더 패턴을 적용 후에 todo 2번을 하려 한다.
...
```
✦ 물론입니다. 컴파일 오류를 직접 해결하셨다니 대단하십니다. 이제 안정적이고 가독성 높은 테스트
  기반이 마련되었으니, 본격적으로 테스트 커버리지를 확장할 최적의 시점입니다.

  시스템의 신뢰도를 높이기 위한 엣지 케이스, 경계값, 예외 상황 테스트에 대한 구체적인 가이드라인을
  각 기능별로 제시해 드리겠습니다.

  1. 예약 생성 (ReservationCreationAcceptanceTest) 기능 보강

  현재 테스트는 기본적인 성공/실패와 유효성 검증에 초점을 맞추고 있습니다. 여기에 다음과 같은
  테스트를 보강할 수 있습니다.

  경계값 테스트

   * 테스트명 제안: "0박 1일(당일 퇴실) 예약은 거부된다"
       * 필요성: 예약 기간의 최소 경계값을 테스트합니다. startDate와 endDate가 동일할 때, 시스템이
         이를 유효하지 않은 기간으로 올바르게 처리하는지 확인합니다.
       * 구현 힌트: aReservationRequest().withStartDate(TODAY).withEndDate(TODAY).build() 와 같이
         시작일과 종료일을 같은 날짜로 설정하여 요청하고, 예약이_거부되었다를 검증합니다.

   * 테스트명 제안: "최소 예약 기간(1박 2일)은 예약이 성공한다"
       * 필요성: 유효한 최소 경계값(1박)이 정상적으로 처리되는지 확인하여, 기간 계산 로직에
         off-by-one 오류가 없는지 검증합니다.
       * 구현 힌트:
         aReservationRequest().withStartDate(TODAY).withEndDate(TODAY.plusDays(1)).build()로
         요청하고 예약_성공_확인을 검증합니다.

   * 테스트명 제안: "과거 날짜로 예약을 생성하면 거부된다"
       * 필요성: 날짜 유효성 검증 로직이 예약 생성 흐름에서도 동작하는지 명시적으로 확인합니다.
       * 구현 힌트: aReservationRequest().withStartDate(TODAY.minusDays(1)).build() 와 같이 과거
         날짜를 설정하고 예약이_거부되었다를 검증합니다.

  엣지 케이스 테스트

   * 테스트명 제안: "기존 예약과 정확히 일치하는 기간에 신규 예약을 시도하면 거부된다"
       * 필요성: 동시성 문제와 별개로, 이미 확정된 예약 위에 순차적으로 덮어쓰려는 시도를 비즈니스
         로직 레벨에서 막는지 확인합니다.
       * 구현 힌트:
           1. GIVEN: 특정 기간으로 예약을 먼저 성공시킵니다.
           2. WHEN: 동일한 사이트와 기간으로 다시 예약을_요청한다.
           3. THEN: 예약이_거부되었다를 검증합니다.

  예외 상황 테스트

   * 테스트명 제안: "존재하지 않는 사이트 ID로 예약하면 거부된다"
       * 필요성: 요청 데이터의 식별자가 유효하지 않을 때, 시스템이 NotFound 또는 BadRequest 오류를
         안정적으로 반환하는지 확인합니다.
       * 구현 힌트: aReservationRequest().withSiteNumber("없는-사이트-99").build() 와 같이 존재하지
         않는 사이트 번호로 요청하고, 예상되는 HTTP 상태 코드와 에러 메시지를 검증합니다.

   * 테스트명 제안: "필수 필드(예약자 이름)가 누락된 경우 예약이 거부된다"
       * 필요성: ReservationRequest DTO에서 @NotNull과 같은 제약 조건이 실제 API 요청에서 올바르게
         동작하는지 검증합니다.
       * 구현 힌트: 빌더에서 .withCustomerName(null)을 설정한 후 요청을 보내고, 400 Bad Request 와
         같은 응답을 검증합니다.

  2. 사이트 검색 (SiteSearchAcceptanceTest) 기능 보강
   ...
   * 테스트명 제안: "기존 예약의 종료일이 검색 시작일과 같으면 해당 사이트는 검색된다"
       * 필요성: 날짜 기반 검색 로직에서 가장 실수가 잦은 off-by-one 에러를 방지합니다. 예약이
         끝나는 날부터 바로 다음 예약이 가능해야 합니다.
       * 구현 힌트:
           1. GIVEN: 오늘+2일 ~ 오늘+4일까지의 예약을 생성합니다.
           2. WHEN: 오늘+4일부터 오늘+6일까지의 기간으로 사이트를 검색합니다.
           3. THEN: 검색 결과에 해당 사이트가 포함되어야 합니다.
   ...
   * 테스트명 제안: "취소된 예약 기간은 다시 검색되었을 때 예약 가능한 상태로 노출된다"
       * 필요성: 예약 취소 로직이 재고(가용 사이트)에 즉시 정확하게 반영되는지 검색 기능을 통해
         이중으로 검증합니다. 이는 두 기능의 상호작용을 테스트하는 중요한 통합 시나리오입니다.
       * 구현 힌트:
           1. GIVEN: 특정 기간으로 사이트를 예약한 후, 즉시 해당 예약을 취소합니다.
           2. WHEN: 원래 예약했던 것과 동일한 기간으로 사이트를 검색합니다.
           3. THEN: 검색 결과에 해당 사이트가 포함되어야 합니다.

  3. 예약 취소 (ReservationCancellationAcceptanceTest) 기능 보강
   ...
   * 테스트명 제안: "존재하지 않는 예약 ID로 취소를 시도하면 실패한다"
       * 필요성: 유효하지 않은 reservationId에 대해 시스템이 404 Not Found와 같은 명확한 응답을
         주는지 확인합니다.
       * 구현 힌트: 예약을_취소한다(99999L, "아무코드") 와 같이 존재하지 않을 법한 ID로 취소를
         요청하고, 404 Not Found 응답을 검증합니다.
  ...
  이 가이드라인을 바탕으로 테스트를 추가하여 시스템을 더욱 견고하게 만들 수 있습니다. 어떤 기능의
  테스트부터 추가해볼까요?

  예를 들어, 가장 간단하면서도 중요한 "예약 생성 기능" 의 경계값 테스트(0박 1일 예약 거부)부터 함께
  추가해보는 것은 어떨까요?
