...
**피드백 반영 내역**
- 테스트 공통 상수 관리
- 동시 예약 테스트의 경우 서비스 통합 테스트로 분리하여 리팩터링 완료 (비관적락 사용)
- 예약 인수 테스트 클래스 분리 
- 성수기(7,8월)에 해당하는 상수와 토요일 구하는 helper method 사용해 가독성 향상

**버그 수정 내역**
- (예약을 취소하기 전) 예약된 사이트가 검색에 노출되는 버그 
- 중간 날짜에 예약된 사이트가 검색에 노출되는 버그 
- 중복 예약 취소 요청시 정상 처리(200)되는 버그 

**궁금한 점**
- 테스트 데이터 빌더를 써보는건 두 번째입니다만, object mother와 혼합해서 사용하기도 하나요?
- 테스트 클래스가 많아지게 되면 패키지도 시나리오별로 분리하는게 좋을까요? 개인적으로 패키지 분리가 어렵습니다 😅
