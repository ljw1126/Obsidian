...
### 10.3. WireMock 응답 맵핑 및 템플릿 가이드

`WireMock`의 응답 맵핑은 `infra/wiremock/mappings/` 디렉토리에 저장된 `.json` 파일을 통해 정의됩니다. 각 `.json` 파일은 특정 HTTP 요청에 대한 응답을 기술합니다.

*   **기본 맵핑:** `request` 객체로 요청 조건을 정의하고, `response` 객체로 반환될 응답(상태 코드, 헤더, 본문)을 지정합니다.
*   **응답 템플릿 (Handlebars):** `bodyFileName` 또는 `jsonBody`와 함께 `response` 객체에 `transformers` 속성을 추가하고 `response-template`을 지정하면, Handlebars 문법을 사용하여 동적인 응답을 생성할 수 있습니다. (예: 요청 본문이나 헤더 값을 참조하여 응답을 동적으로 구성)
...
### 10.4. WireMock Admin API 접근

`WireMock` 서버는 테스트 및 디버깅을 위한 관리 API를 제공합니다.

*   **관리 UI (브라우저):** `http://localhost:8084/__admin/webapp/`
    *   현재 로드된 맵핑 목록, 수신된 요청 로그, 기록된 스냅샷 등을 웹 UI에서 확인할 수 있습니다.
*   **API 엔드포인트 (curl):** `http://localhost:8084/__admin/`
    *   `curl`과 같은 도구를 사용하여 관리 API에 직접 요청을 보낼 수 있습니다. 예를 들어, 모든 맵핑 목록을 조회하거나, 요청 로그를 리셋하는 등의 작업을 수행할 수 있습니다.

    ```bash
    # 모든 맵핑 목록 조회
    curl -X GET http://localhost:8084/__admin/mappings

    # 모든 요청 로그 조회
    curl -X GET http://localhost:8084/__admin/requests

    # 요청 로그 초기화
    curl -X POST http://localhost:8084/__admin/requests/reset
    ```

    이러한 Admin API를 활용하면 테스트 시나리오에서 `WireMock`의 상태를 프로그래밍 방식으로 제어하고 검증하는 데 유용합니다.