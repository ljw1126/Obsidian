...
```text
설계와 테스트 역량 강화를 목표로 자기계발한 경험을 설명하겠습니다.

절차적 코드만을 추구하는 환경에서 일을 했었는데, 요구사항 변화가 많을 수록 코드의 복잡성이 올라가 유지보수 비용 증가하여 관리가 어려워진다는 것을 경험했었습니다. 이를 통해 테스트와 설계 역량의 필요성을 깨닫게 되었고, 주위 사람과 외부 스터디 등을 해보았지만 결과적으로 스스로 전문성을 키워 경쟁력을 갖춰야 한다는 사실만을 깨달았습니다 

그래서 20개 도서, 강의 등을 활용해 기본기와 사용 기술에 대한 깊이있는 이해를 쌓기 위해 자료구조와 알고리즘 부터 시작하여 학습 테스트를 활용했습니다.
TDD 클린코드 교육을 찾아 신청했었는데 80만원이라 비싸긴 했지만 주도적으로 과제 수행한 덕분에 단위 테스트와 도메인 설계 역량을 익힐 수 있었다. 이후에 이해가 안되었던 테스트 강의들을 다시 들었을때 모호했던 부분(퍼즐 조각)이 채워져서 이해도가 상승하였고, 레이어별 테스트, 통합 테스트, 부하 테스트와 모니터링 등 테스트 방법 체계화하고 외부 의존성 제어하는 방법 5가지 등을 습득할 수 있었습니다.

그리고 개인 프로젝트로 선착순 쿠폰 발급 시스템을 만들어 지식을 적용해 보았습니다. 부하 테스트와 모니터링 환경을 docker로 구축하고, 동시성 제어와 많은 트래픽에 대한 병목 지점을 파악하여 리팩토링하여 성능 개선하였습니다. 그 결과 RPS 성능 6배, 평균 응답시간 26배 상승 경험을 쌓을 수 있었습니다. 또한 Testcontainer, kafka 도입 했을때 성능 비교나 부하 테스트 툴을 여러 가지 써보면서 장단점을 비교해보며 기록하고 경험을 쌓았습니다.

컴퓨터가 이해할 수 있는 코드는 누구나 작성가능하지만, 사람이 이해할 수 잇는 코드는 좋을 개발자만이 가능하다는 말이 있습니다. 이처럼 읽기 편한 좋은 코드를 작성하기 위해, 안정적으로 운영가능한 역량을 쌓기 위해 지금까지 노력해온 만큼, 조직과 팀에 기여하고 싶습니다

---
chat-gpt
**1️⃣ 배경 및 필요성 인식**  
_"과거에는 절차적 코드만을 추구하는 환경에서 일했는데, 요구사항 변화가 많아질수록 코드의 복잡성이 증가하고 유지보수 비용이 높아지는 문제를 경험했습니다. 이를 통해 테스트와 설계 역량이 필수적이라는 것을 깨닫게 되었습니다._

_처음에는 주위 사람들과 논의하고 외부 스터디에 참여했지만, 궁극적으로 스스로 전문성을 키워야 한다는 결론에 도달했습니다."_

**2️⃣ 학습 과정 및 방법**  
_"이에 따라, 자료구조 및 알고리즘부터 학습 테스트를 활용하여 기본기를 다졌습니다. 이후, TDD 및 클린 코드 교육을 수강하며(비용이 80만 원이었지만) 과제를 주도적으로 수행한 덕분에 단위 테스트 및 도메인 설계 역량을 익혔습니다._

_특히, 레이어별 테스트, 통합 테스트, 부하 테스트 및 모니터링을 체계화하고, 외부 의존성을 제어하는 방법(5가지 패턴) 등을 학습했습니다."_

**3️⃣ 실전 적용 및 성과**  
_"이러한 학습 내용을 개인 프로젝트로 ‘선착순 쿠폰 발급 시스템’을 개발하였습니다.  
Docker 기반으로 부하 테스트 및 모니터링 환경을 구축하고, 동시성 제어 및 병목 지점을 분석하여 리팩토링을 진행했습니다.  
그 결과, **RPS 성능 6배 향상, 평균 응답시간 26배 개선**이라는 성과를 얻을 수 있었습니다._

_또한, Testcontainer와 Kafka 도입 시 성능 비교 및 다양한 부하 테스트 도구를 활용하여 실험하면서, 각각의 장단점을 기록하고 체계적으로 정리하였습니다."_

**4️⃣ 마무리 (철학 & 기여 포인트 강조)**  
_"'컴퓨터가 이해할 수 있는 코드는 누구나 작성할 수 있지만, 사람이 이해할 수 있는 코드는 좋은 개발자만이 가능하다'는 말이 있습니다.  
이처럼, 유지보수성과 확장성을 고려한 설계를 기반으로 안정적인 서비스를 운영할 수 있는 역량을 쌓아왔으며, 이러한 경험을 바탕으로 팀과 조직에 기여하고 싶습니다."_
...
기존 ec2 인프라 구성 분석, 필요한 AWS 조사
Dockerfile 빌드 테스트 
운영 환경변수 관리, 보안 위해 AWS Secrets Manager 도입 결정
스프링 부트 버전 2.1.6 > 2.7.1 업한후 의존성 문제 해결하고 환경변수 호출 테스트 
서브 도메인 발급 받아 Jenkins ~ ECS 까지 전체 연결 구성 및 이슈 해결
사내 공지 후 점진적으로 3개 서비스 이관, 운영환경 이슈 해결 후 전체 내용 문서화하여 Confluence 공유

트러블 슈팅 6개 정도 있었는데 한개만 설명하겟습니다.
- 500 Internal Server error, 버전업에 따른 security seiralUID 문제

성과 
- 컨테이너 자동 복구 기능 통해 고가용성 확보, CI/CD 자동화 통해 배포시 끊김없는 서비스 제공하며 사용자 경험 개선
- AWS 아키텍처 설계 역량 강화, 역량 평가 8명중 1위
...
레벨 디자인팀의 오픈 모니터링 시스템 

- 의사소통 능력의 중요성을 깨달음 
- 변화에 유연하게 대응할 수 있는 설계와 테스트 역량의 중요성 깨달음 
...
```text
제가 중요하게 생각하는 가치는 세 가지입니다. 첫째, 완벽함은 존재하지 않고 변화는 막을 수 없다는 점입니다. 따라서 다양한 기술과 도구를 익히고, 상황에 맞는 최선의 선택을 하며 트레이드오프를 고려해야 한다고 생각합니다.

둘째, 개발자는 비즈니스 문제를 해결하고 가치를 창출하는 직무라는 점입니다. 이를 위해 코드 품질과 유지보수성을 높이는 것이 중요하다고 생각합니다.

실제로, 실무에서 절차적 코드 작성만을 추구했던 경험이 있습니다. 당시 코드가 변화에 유연하게 대응하지 못해 유지보수 비용이 증가하는 문제를 겪었고, 이를 해결하기 위해 코드 설계와 테스트의 중요성을 깨닫게 되었습니다. 이후, 단위 테스트와 도메인 설계, 부하 테스트와 모니터링 등 오픈 소스 활용 역량을 자기계발을 통해 쌓았습니다.

/*
이러한 역량을 바탕으로 선착순 쿠폰 발급 시스템 프로젝트에서 실제로 적용해볼 기회가 있었습니다. 많은 트래픽 속에서도 시스템을 안정적으로 운영하기 위해 사전 부하 테스트를 수행하고, 성능 모니터링을 통해 병목 지점을 분석하며 개선했습니다. 그 결과, 기존보다 성능이 RPS 5배, 평균 응답 시간 26배 정도 크게 향상되었습니다. 여기서 끝내기보다 다른 부하 테스트 툴을 사용해보고 비교해보거나, kafka와 testcontainer 도입시 얼마나 성능 개선 될까와 같은 호기심 탐구하며 시스템 개발과 유지보수에 필요한 역량을 쌓았습니다. 
*/

마지막으로, 하드 스킬 뿐만 아니라 소프트 스킬도 중요하다 생각합니다. 개발자는 고객의 문제를 해결하는 직무이므로, 원활한 커뮤니케이션과 리더십이 필요합니다. 그리고 혼자서 해결할 수 없는 경우 전문가의 자문을 구하거나 협업을 통해 문제를 해결해야 합니다. 이러한 철학을 바탕으로 전 회사에서 여러 역할을 맡아 다양한 구성원을 이끌어 결과물을 만들었습니다.(게임 운영에 필요한 파이프라인을 구축하고 생산성 향상과 각 부서별 업무 효율성 향상게 기여할 수 있었습니다.) 특히 커피타임을 활용해서 방향성을 명확히하고 시작하는게 중요하다는 사실을 깨달았습니다.

이제는 이러한 경험을 바탕으로, 기술적 깊이뿐만 아니라 팀과 조직에 기여할 수 있는 개발자가 되고 싶습니다.
